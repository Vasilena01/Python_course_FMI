...
class BaseMaterialsTests(unittest.TestCase):
    ...
    def test_for_volume_decimal_places_if_more_than_places_up(self):
        ...
        steel, = self.factory(Steel=expected_density)
        self.assertEqual(steel.volume, 0.58)
        self.assertIsInstance(steel.volume, float)
    ...
    def test_for_volume_decimal_places_if_more_than_places_down(self):
        ...
        self.assertEqual(steel.volume, 7.42)
        self.assertIsInstance(steel.volume, float)
...
class FactoryTests(unittest.TestCase):
    ...
    def tearDown(self) -> None:
        """за да пишете тестове за последната функция трябва да си
        изчистите стейта на целия клас тук което зависи от конректната имплементация, затова няма да пиша тестове за
        Factory.can_build_together
        """
    ...
    def test_volume_for_all_instances_returned_from_factory_call_kwargs(self):
        ...
        self.assertEqual(wood.volume, 1.0)
        self.assertEqual(steel.volume, 0.01)
        self.assertEqual(stone.volume, 2.0)
    ...
    def test_new_generated_class_name_is_sorted_in_ascending_order_with_repeating_combinations(self):
        result1 = self.factory1(self.wood1, self.steel2, self.concrete1)
        result2 = self.factory1(self.steel1, self.wood2, self.brick1)
        result = self.factory1(result1, result2)
        self.assertEqual(result.__class__.__name__, "Brick_Concrete_Steel_Wood")
    ...
    def test_volume_and_density_for_dynamically_created_materials(self):
        ...
        self.assertEqual(result.volume, 0.57)

    def test_volume_and_density_for_dynamically_created_and_recombined_materials(self):
        # result density = (600 + 7700 + 2000 + 2500) = 12800 / 4 = 3200.0
        # result mass = (5 + 15) + (5 + 60 + 2000) = 2085
        # result volume = 2085 / 3200 = 0.6515625 = 0.65
        result1 = self.factory1(self.wood1, self.steel1)
        result2 = self.factory1(self.wood2, self.brick1, self.concrete2)
        result = self.factory1(result1, result2)
        self.assertEqual(result.volume, 0.65)
    ...
    def test_build_method_with_many_materials_returns_true(self):
        ...
        # volume = 73000 / 3433.3333333333335 = 21.262135922330096 = 21.26
        factory1 = Factory()
