import unittest
import ast
from Homework3 import Tone, Interval, Chord
 
# Replace with the path to the file
HOMEWORK_FILENAME = 'Homework3.py'
...
class TestChord(unittest.TestCase):
    ...
    def test_init(self):
        ...
        for index, tone in enumerate(c_minor_chord.chord):
        ...
        for index, tone in enumerate(csus4_chord.chord):
            self.assertEqual(str(tone), str(expected_tones[index]))
        ...
        for index, tone in enumerate(f_sixth_ninth_chord.chord):
            self.assertEqual(str(tone), str(expected_tones[index]))
